...
object effect {
  ...
  def toIO[A](future: => Future[A])(implicit ec: ExecutionContext): IO[A] = {
    IO.async { k =>
      //future.onComplete((t: Try[A]) => k(t.toEither)) // other version Luka
      future.onComplete {
        case Success(s) => k(Right(s))
        case Failure(f) => k(Left(f))
      }
  ...
  def readFile(file: File): Resource[IO, List[String]] = {
    val acquire: IO[BufferedReader] =
      printLine(s"Acquring file: $file") *> IO(
        new BufferedReader(new FileReader(file)))

    Resource
      .make(acquire)(in =>
        printLine(s"Releasing file handle of: $file") *> IO(in.close()))
